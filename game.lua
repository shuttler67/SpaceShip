function game_load()
    player = Player()
    player:setCameraslack(love.window.getWidth()/4, love.window.getHeight()/4)
    ...
    modules:addModule(ThrusterModule(ShipModule.WEST), -1, 0)
    modules:addModule(ThrusterModule(ShipModule.EAST), 1, 0)
    modules:addModule(ThrusterModule(ShipModule.EAST), 1, -1)
    modules:addModule(ThrusterModule(ShipModule.SOUTH), 0, 1)
